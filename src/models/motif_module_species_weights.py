...
class MotifLitModule(LightningModule):
    ...
    def step(self, batch: Any):
        ...
        logits, embeddings = self.forward(x[0],x[1])
        ...
        return loss, preds, logits, embeddings
    ...
    def test_step(self, batch: Any, batch_idx: int):
        loss, preds, logits, embeddings = self.step(batch)
        targets = batch[2]
        targets_masked = batch[1]
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        seq_embeddings = torch.zeros((len(outputs),len(outputs[0]["sequence_embedding"])))
        for i in range(len(outputs)):
            seq_embeddings[i] = outputs[i]["sequence_embedding"]
        torch.save(seq_embeddings,"seq_embeddings.torch")
        np.save("seq_embeddings.npy",seq_embeddings.numpy())
